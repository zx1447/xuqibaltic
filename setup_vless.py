#!/usr/bin/env python3
"""Parse FLARELAX_VLESS_URI and write a local Xray SOCKS5 client config."""
import html
import json
import os
import sys
...
raw = html.unescape(os.environ.get("FLARELAX_VLESS_URI", "").strip())
if not raw:
    sys.exit("FLARELAX_VLESS_URI is empty")
uri = urlsplit(raw)
if uri.scheme.lower() != "vless" or not uri.username or not uri.hostname:
    sys.exit("Invalid FLARELAX_VLESS_URI")
...
output = os.environ.get("XRAY_CONFIG", "/tmp/flarelax-xray/config.json")
